...
async function ipDup(ip:string, port:number){ return (await getAllVPS()).some(v=>v.ip===ip && v.port===port); }
async function portOK(ip:string, port:number){
  try{ const conn = await Deno.connect({hostname: ip.replace(/^\[|\]$/g,''), port, transport:'tcp'}); conn.close(); return true; }catch{ return false; }
}
async function addVPS(server: Omit<VPSServer,'id'>){
  ...
  const list = r.value || []; list.push(v.id);
  await kv.set(['user_donations', v.donatedBy], list);
  return v;
}
async function delVPS(id:string){
  const r = await kv.get<VPSServer>(['vps', id]); if(!r.value) return false;
...
async function updVPSStatus(id:string, s:VPSServer['status']){
  const r = await kv.get<VPSServer>(['vps', id]); if(!r.value) return false;
  r.value.status = s; await kv.set(['vps', id], r.value); return true;
}
...
async function getSession(id: string){
  const r = await kv.get<Session>(['sessions', id]); if(!r.value) return null;
...
async function createSession(userId:string, username:string, avatarUrl:string|undefined, isAdmin:boolean){
  ...
  await kv.set(['sessions', s.id], s); return s.id;
}
async function getUser(linuxDoId:string){ return (await kv.get<User>(['users', linuxDoId])).value || null; }
async function upsertUser(linuxDoId:string, username:string, avatarUrl?:string){
  ...
  await kv.set(['users', linuxDoId], u); return u;
}
...
const requireAuth = async(c:any, next:any)=>{
  const sid = getCookie(c,'session_id'); if(!sid) return c.json({success:false,message:'未登录'},401);
  const s = await getSession(sid); if(!s) return c.json({success:false,message:'会话已过期'},401);
  c.set('session', s); await next();
};
const requireAdmin = async(c:any,next:any)=>{
  const sid = getCookie(c,'admin_session_id'); if(!sid) return c.json({success:false,message:'未登录'},401);
...
app.get('/oauth/callback', async c=>{
  const code = c.req.query('code'); const error = c.req.query('error'); const state = c.req.query('state') || '/donate';
  if(error){ return c.html(`<!doctype html><body><h1>登录失败</h1><p>${error}</p><a href="/donate">返回</a></body>`); }
  if(!code) return c.text('Missing code',400);
  try{
    const cfg = await getOAuth(); if(!cfg) return c.html('<!doctype html><body><h1>配置错误</h1><a href="/donate">返回</a></body>');
...
app.get('/api/logout', async c=>{
  const sid = getCookie(c,'session_id'); if(sid) await kv.delete(['sessions', sid]);
...
app.get('/api/user/donations', requireAuth, async c=>{
  const s = c.get('session'); const ids = (await kv.get<string[]>(['user_donations', s.userId])).value || [];
  const arr: VPSServer[] = [];
  for(const id of ids){ const r = await kv.get<VPSServer>(['vps', id]); if(r.value) arr.push(r.value); }
  const safe = arr.sort((a,b)=>b.donatedAt-a.donatedAt).map(d=>({
    id:d.id, ip:d.ip, port:d.port, username:d.username, authType:d.authType, donatedAt:d.donatedAt,
    status:d.status, note:d.note, country:d.country, traffic:d.traffic, expiryDate:d.expiryDate,
    specs:d.specs, ipLocation:d.ipLocation, verifyStatus:d.verifyStatus, lastVerifyAt:d.lastVerifyAt,
...
app.put('/api/user/donations/:id/note', requireAuth, async c=>{
  const s = c.get('session'); const id = c.req.param('id'); const {note} = await c.req.json();
  const r = await kv.get<VPSServer>(['vps', id]); if(!r.value) return c.json({success:false,message:'VPS 不存在'},404);
  if(r.value.donatedBy !== s.userId) return c.json({success:false,message:'无权修改'},403);
  r.value.note = (note||'').toString(); await kv.set(['vps', id], r.value); return c.json({success:true,message:'备注已更新'});
});
...
app.post('/api/donate', requireAuth, async c=>{
  ...
  const p = parseInt(String(port),10); if(p<1||p>65535) return c.json({success:false,message:'端口 1~65535'},400);
  if(await ipDup(ip,p)) return c.json({success:false,message:'该 IP:端口 已被投喂'},400);
  if(!(await portOK(ip,p))) return c.json({success:false,message:'无法连接到该服务器'},400);
  ...
  const v = await addVPS({
    ...
    donatedBy: s.userId, donatedByUsername: s.username, donatedAt: Date.now(),
    status: 'active',  // 存储仍为 active，前端展示“运行中”
    note: note||'', adminNote:'', country, traffic, expiryDate, specs,
    ipLocation: ipLoc, verifyStatus:'verified', lastVerifyAt: Date.now()
  });
  return c.json({success:true,message:'✅ 投喂成功，已自动验证并标记运行中', data:{id:v.id, ipLocation: v.ipLocation}});
});
...
app.post('/api/admin/login', async c=>{
  const {password} = await c.req.json(); const real = await getAdminPwd();
...
app.get('/api/admin/logout', async c=>{
  const sid = getCookie(c,'admin_session_id'); if(sid) await kv.delete(['sessions', sid]);
  setCookie(c,'admin_session_id','',{maxAge:0,path:'/'}); return c.json({success:true});
});
app.get('/api/admin/vps', requireAdmin, async c=> c.json({success:true,data: await getAllVPS()}));
app.delete('/api/admin/vps/:id', requireAdmin, async c=>{
  const ok = await delVPS(c.req.param('id')); return c.json(ok?{success:true,message:'VPS 已删除'}:{success:false,message:'不存在'}, ok?200:404);
});
app.put('/api/admin/vps/:id/status', requireAdmin, async c=>{
  ...
  const ok = await updVPSStatus(id,status); return c.json(ok?{success:true,message:'状态已更新'}:{success:false,message:'不存在'}, ok?200:404);
});
app.put('/api/admin/vps/:id/notes', requireAdmin, async c=>{
  const id = c.req.param('id'); const {note,adminNote,country,traffic,expiryDate,specs} = await c.req.json();
  const r = await kv.get<VPSServer>(['vps', id]); if(!r.value) return c.json({success:false,message:'不存在'},404);
...
app.get('/api/admin/config/oauth', requireAdmin, async c=> c.json({success:true,data: (await getOAuth())||{}}));
app.put('/api/admin/config/oauth', requireAdmin, async c=>{
  ...
  await setOAuth({clientId,clientSecret,redirectUri}); return c.json({success:true,message:'OAuth 配置已更新'});
});
app.put('/api/admin/config/password', requireAdmin, async c=>{
  const {password} = await c.req.json(); if(!password || String(password).length<6) return c.json({success:false,message:'密码至少 6 位'},400);
  await setAdminPwd(String(password)); return c.json({success:true,message:'管理员密码已更新'});
});
app.get('/api/admin/stats', requireAdmin, async c=>{
  ...
  const top = Array.from(userStats.entries()).map(([username,count])=>({username,count})).sort((a,b)=>b.count-a.count).slice(0,10);
...
app.post('/api/admin/vps/:id/mark-verified', requireAdmin, async c=>{
  const id = c.req.param('id'); const r = await kv.get<VPSServer>(['vps', id]); if(!r.value) return c.json({success:false,message:'不存在'},404);
  r.value.verifyStatus='verified'; r.value.status='active'; r.value.lastVerifyAt=Date.now();
  await kv.set(['vps', id], r.value); return c.json({success:true,message:'已标记为验证通过'});
});
...
function modalEdit(title, fields, onOk){
  ...
  fields.forEach(f=>{ const box=document.createElement('div'); const lab=document.createElement('div'); lab.className='muted text-xs mb-1'; lab.textContent=f.label; 
    ...
    if(f.type!=='textarea') inp.type='text'; (inp as any).value=f.value||''; (inp as any).placeholder=f.placeholder||''; (inp as any).rows= f.type==='textarea'?3:undefined;
    ...
    box.appendChild(lab); box.appendChild(inp); (box as any)._get=()=>inp.value; (box as any)._key=f.key; form.appendChild(box); });
  ...
  btn2.onclick=()=>{ const data:any={}; form.childNodes.forEach((n:any)=>{ data[n._key]=n._get(); }); try{ onOk(data,()=>wrap.remove()); }catch(e){ console.error(e); } };
...
async function gotoDonatePage(){
  try{
    ...
    if(r.ok){ location.href='/donate/vps'; }else{ location.href='/oauth/login?redirect='+encodeURIComponent('/donate/vps'); }
  }catch{ location.href='/oauth/login?redirect='+encodeURIComponent('/donate/vps'); }
}
...
async function loadLeaderboard(){
  ...
  try{
    const res = await fetch('/api/leaderboard',{credentials:'same-origin',cache:'no-store'}); const j = await res.json();
    if(!res.ok||!j.success){ box.innerHTML='<div class="text-red-400 text-sm">加载失败</div>'; return; }
    const data=j.data||[]; countEl.textContent = data.length?(' · 共 '+data.length+' 位投喂者'):'';
    if(!data.length){ box.innerHTML='<div class="muted text-sm">暂时还没有投喂记录，成为第一个投喂者吧～</div>'; return; }
    box.innerHTML='';
    data.forEach((it,idx)=>{
      ...
      '<div class="muted text-xs">共投喂 '+it.count+' 台 VPS</div>'; wrap.appendChild(head);
  ...
  }catch{ box.innerHTML='<div class="text-red-400 text-sm">加载异常</div>'; }
}
loadLeaderboard();
</script>
</body></html>`;
...
/* ==================== /donate/vps （投喂中心：表单示例 + 日期默认+1年 + Toast） ==================== */
...
function bindAuthType(){
  const sel=document.querySelector('select[name="authType"]'); const pwd=document.getElementById('password-field'); const key=document.getElementById('key-field');
  sel.addEventListener('change',()=>{ if(sel.value==='password'){ pwd.classList.remove('hidden'); key.classList.add('hidden'); }else{ pwd.classList.add('hidden'); key.classList.remove('hidden'); } });
}
...
async function submitDonate(e){
  ...
  const payload={ ip:fd.get('ip')?.toString().trim(), port:Number(fd.get('port')||''), username:fd.get('username')?.toString().trim(),
  ...
  try{
    ...
    const j=await r.json(); if(!r.ok||!j.success){ msg.textContent=j.message||'提交失败'; toast('投喂失败：'+(j.message||'请检查填写项'), 'error'); }
    else{ msg.textContent=j.message||'投喂成功'; toast(j.message||'投喂成功','success'); form.reset(); loadDonations(); }
  }catch(e){ msg.textContent='提交异常'; toast('提交异常','error'); } finally{ btn.disabled=false; btn.textContent=t; }
}
async function loadDonations(){
  const box=document.getElementById('donations-list'); box.innerHTML='<div class="muted text-xs">正在加载...</div>';
  try{
    ...
    const data=j.data||[]; if(!data.length){ box.innerHTML='<div class="muted text-xs">还没有投喂记录，先在左侧提交一台吧～</div>'; return; }
    box.innerHTML='';
    data.forEach(v=>{
...
ensureLogin(); bindAuthType(); document.getElementById('donate-form').addEventListener('submit', submitDonate); loadDonations();
</script>
</body></html>`;
  return c.html(html);
});

/* ==================== /admin （后台：登录修复+展开OAuth+站内弹窗+编辑信息框+运行中文案） ==================== */
app.get('/admin', c=>{
...
async function checkAdmin(){
  ...
  let timeout = setTimeout(()=>{ renderLogin(root); }, 2500); // 兜底
  try{
    const r = await fetch('/api/admin/check-session',{credentials:'same-origin',cache:'no-store'});
    clearTimeout(timeout);
    const j = await r.json().catch(()=>({success:false}));
    if(!j.success || !j.isAdmin){ renderLogin(root); } else { renderAdmin(root, j.username); await loadStats(); await loadConfig(); await loadVps(); }
  }catch(e){ clearTimeout(timeout); renderLogin(root); }
}
...
function renderLogin(root){
  ...
  document.getElementById('admin-login-form').addEventListener('submit', async(e)=>{
    e.preventDefault(); const fd=new FormData(e.target); const pwd=fd.get('password')?.toString()||'';
    try{
      ...
      const j=await r.json(); if(!r.ok||!j.success){ toast(j.message||'登录失败','error'); } else { toast('登录成功','success'); location.reload(); }
    }catch{ toast('登录异常','error'); }
  });
}

function renderAdmin(root, name){
  root.innerHTML='';
  const header=document.createElement('header'); header.className='mb-6 flex items-start justify-between gap-4';
  ...
  root.appendChild(header); updateThemeBtn(); document.getElementById('theme-toggle').addEventListener('click',toggleTheme);
  ...
  const cfg=document.createElement('section'); cfg.id='admin-config'; cfg.className='mt-4'; cfg.innerHTML=
  ...
  listWrap.innerHTML='<div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-2">'+
    ...
    '</div></div>'+
    '<div class="flex justify-end mb-2"><button id="btn-batch-verify" class="text-[11px] rounded-lg border px-3 py-1">🛠 一键验证 VPS 状态</button></div>'+
  ...
  document.getElementById('filter-btn').addEventListener('click',()=>{ searchFilter=(document.getElementById('filter-input') as HTMLInputElement).value.trim(); userFilter=''; renderVpsList(); });
  document.getElementById('filter-clear-btn').addEventListener('click',()=>{ searchFilter=''; (document.getElementById('filter-input') as HTMLInputElement).value=''; userFilter=''; renderVpsList(); });
  document.getElementById('btn-batch-verify').addEventListener('click', async()=>{
    try{ const r=await fetch('/api/admin/vps/batch-verify',{method:'POST',credentials:'same-origin'}); const j=await r.json(); toast(j.message||'验证完成','success'); }catch{ toast('批量验证失败','error'); }
    await loadVps(); await loadStats();
  });
}

async function loadStats(){
  const wrap=document.getElementById('admin-stats'); wrap.innerHTML='<div class="muted text-xs mb-3">正在加载统计信息...</div>';
  try{
    const r=await fetch('/api/admin/stats',{credentials:'same-origin',cache:'no-store'}); const j=await r.json();
    if(!r.ok||!j.success){ wrap.innerHTML='<div class="text-red-400 text-xs">统计信息加载失败</div>'; return; }
    const d=j.data||{};
    function card(l,v,k){ return '<button data-gok="'+k+'" class="rounded-2xl border px-3 py-2 text-left">'+
    ...
    wrap.innerHTML='<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-3">'+
      card('总投喂数',d.totalVPS||0,'all')+card('运行中',d.activeVPS||0,'active')+card('未启用',d.inactiveVPS||0,'inactive')+card('失败',d.failedVPS||0,'failed')+card('待验证',d.pendingVPS||0,'pending')+card('今日新增',d.todayNewVPS||0,'all')+'</div>';
...
async function loadConfig(){
  ...
  const f=document.getElementById('oauth-form') as HTMLFormElement;
...
async function saveOAuth(){
  const f=document.getElementById('oauth-form') as HTMLFormElement;
  const payload={ clientId:(f.querySelector('input[name="clientId"]') as HTMLInputElement).value.trim(),
  ...
  try{ const r=await fetch('/api/admin/config/oauth',{method:'PUT',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
    const j=await r.json(); if(!r.ok||!j.success){ toast(j.message||'保存失败','error'); } else { toast('已保存','success'); } }catch{ toast('保存异常','error'); }
}
async function loadVps(){
  const list=document.getElementById('vps-list'); list.innerHTML='<div class="muted text-xs">正在加载 VPS...</div>';
  try{ const r=await fetch('/api/admin/vps',{credentials:'same-origin',cache:'no-store'}); const j=await r.json(); if(!r.ok||!j.success){ list.innerHTML='<div class="text-red-400 text-xs">加载失败</div>'; return; }
    allVpsList=j.data||[]; renderVpsList(); }catch{ list.innerHTML='<div class="text-red-400 text-xs">加载异常</div>'; }
}
  ...
  arr.forEach(v=>{
    ...
    card.querySelectorAll('button[data-act]').forEach(btn=>{
      ...
      btn.addEventListener('click', async()=>{
        if(!id) return;
        if(act==='mark'){ try{ const r=await fetch('/api/admin/vps/'+id+'/mark-verified',{method:'POST',credentials:'same-origin'}); const j=await r.json(); toast(j.message||'已标记','success'); }catch{ toast('操作失败','error'); } }
        else if(act==='inactive'||act==='failed'){ try{ const r=await fetch('/api/admin/vps/'+id+'/status',{method:'PUT',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({status:act})}); const j=await r.json(); toast(j.message||'已更新','success'); }catch{ toast('更新失败','error'); } }
        else if(act==='del'){ try{ const r=await fetch('/api/admin/vps/'+id,{method:'DELETE',credentials:'same-origin'}); const j=await r.json(); toast(j.message||'已删除', r.ok?'success':'error'); }catch{ toast('删除失败','error'); } }
        else if(act==='edit'){ 
          modalEdit('编辑 VPS 信息（用户备注前台可见）',[
            ...
            try{ const r=await fetch('/api/admin/vps/'+id+'/notes',{method:'PUT',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)}); const j=await r.json(); if(!r.ok||!j.success){ toast(j.message||'保存失败','error'); }else{ toast('已保存','success'); close(); await loadVps(); } }catch{ toast('保存异常','error'); }
          });
        }
        await loadVps(); await loadStats();
      });
    });
    const link=card.querySelector('a[href^="https://linux.do/u/"]'); link?.addEventListener('click',e=>{ e.preventDefault(); userFilter=v.donatedByUsername; renderVpsList(); });
