...
async function addVPS(server: Omit<VPSServer, 'id'>) {
  ...
  // 触发自动部署 webhook (await 保证执行, 10s超时)
  try {
    const ctrl = new AbortController();
    ...
    clearTimeout(timer);
    log(`[Webhook] 触发部署: ${wRes.status}`);
  } catch (e: any) {
    log(`[Webhook] 触发失败: ${e?.message || e}`);
  }
  return v;
}
...
.btn-primary {
  ...
  padding: 0.75rem 1.5rem; border-radius: var(--radius-sm); font-weight: 600; font-size: 0.875rem;
...
.btn-secondary {
  display: inline-flex; align-items: center; justify-content: center; gap: 0.5rem;
  padding: 0.625rem 1.25rem; border-radius: var(--radius-sm); font-weight: 500; font-size: 0.875rem;
...
.btn-danger {
  display: inline-flex; align-items: center; justify-content: center; gap: 0.5rem;
  padding: 0.5rem 1rem; border-radius: var(--radius-xs); font-weight: 500; font-size: 0.8rem;
  background: rgba(255,69,58,0.12); color: var(--error);
  border: 1px solid rgba(255,69,58,0.2); cursor: pointer; transition: all 0.2s ease;
}
.btn-danger:hover { background: rgba(255,69,58,0.2); }

.btn-sm { padding: 0.375rem 0.75rem; font-size: 0.75rem; border-radius: var(--radius-xs); }
...
.input-field.error { border-color: var(--error); box-shadow: 0 0 0 3px rgba(255,69,58,0.12); }
.input-field.success { border-color: var(--success); box-shadow: 0 0 0 3px rgba(48,209,88,0.12); }
...
/* Badges */
.badge { display: inline-flex; align-items: center; gap: 0.375rem; padding: 0.25rem 0.75rem; border-radius: 9999px; font-size: 0.7rem; font-weight: 600; letter-spacing: 0.02em; }
.badge-ok { background: rgba(48,209,88,0.12); color: #30D158; border: 1px solid rgba(48,209,88,0.2); }
.badge-fail { background: rgba(255,69,58,0.12); color: #FF453A; border: 1px solid rgba(255,69,58,0.2); }
.badge-idle { background: rgba(142,142,147,0.12); color: #8E8E93; border: 1px solid rgba(142,142,147,0.2); }
.badge-warn { background: rgba(255,159,10,0.12); color: #FF9F0A; border: 1px solid rgba(255,159,10,0.2); }
.badge-info { background: rgba(0,122,255,0.12); color: #0A84FF; border: 1px solid rgba(0,122,255,0.2); }
...
.animate-slide-up { animation: slide-up 0.5s ease forwards; opacity: 0; }
...
/* Skeleton */
.skeleton { background: linear-gradient(90deg, rgba(255,255,255,0.04) 25%, rgba(255,255,255,0.08) 50%, rgba(255,255,255,0.04) 75%); background-size: 200% 100%; animation: skeleton-loading 1.5s ease-in-out infinite; border-radius: var(--radius-xs); }
...
/* Alerts */
.alert-info { background: rgba(0,122,255,0.08); border: 1px solid rgba(0,122,255,0.15); border-radius: var(--radius-sm); padding: 1rem; color: #64D2FF; }
.alert-warning { background: rgba(255,159,10,0.08); border: 1px solid rgba(255,159,10,0.15); border-radius: var(--radius-sm); padding: 1rem; color: #FFD60A; }

/* Modal */
.modal-overlay { position: fixed; inset: 0; z-index: 50; display: flex; align-items: center; justify-content: center; padding: 1rem; background: rgba(0,0,0,0.65); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); }
.modal-content { width: 100%; max-width: 40rem; max-height: 90vh; overflow-y: auto; background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: var(--radius); box-shadow: 0 20px 60px rgba(0,0,0,0.5); }

/* Copyable field */
.copy-field { cursor: pointer; transition: all 0.15s ease; position: relative; }
.copy-field:hover { background: var(--bg-elevated) !important; }
.copy-field:active { transform: scale(0.98); }
...
@media (max-width: 640px) {
  ...
  .btn-primary, .btn-secondary { padding: 0.625rem 1rem; font-size: 0.8rem; }
  #toast-root { top: 0.75rem; right: 0.75rem; left: 0.75rem; }
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 py-8 md:py-16">
  ...
  <header class="text-center mb-16 animate-fade-in">
    ...
    <h1 class="text-4xl md:text-6xl font-extrabold text-white mb-4" style="letter-spacing:-0.03em">VPS 投喂榜</h1>
    <p class="text-lg text-[#a1a1a6] max-w-2xl mx-auto">感谢每一位无私奉献的赞助商，共建公益机场节点网络</p>
  ...
  filtered.forEach(v=>{
    ...
    card.querySelectorAll('button[data-act]').forEach(btn=>{
      btn.addEventListener('click',async()=>{
        ...
        else if(act==='verify'){btn.textContent='验证中...';btn.disabled=true;try{const r=await fetch('/api/admin/vps/'+v.id+'/verify',{method:'POST',credentials:'same-origin'});const j=await r.json();toast(j.message||'已验证',j.success?'success':'error');if(j.data){const ix=allVpsList.findIndex(x=>x.id===v.id);if(ix>=0){allVpsList[ix].status=j.data.status;allVpsList[ix].verifyStatus=j.data.verifyStatus;allVpsList[ix].verifyErrorMsg=j.data.verifyErrorMsg||'';allVpsList[ix].lastVerifyAt=j.data.lastVerifyAt;}renderVpsList();}}catch{toast('验证异常','error');}}
...
function showConfigModal(v) {
  ...
  overlay.innerHTML = '<div class="modal-content" style="max-width:44rem"><div class="p-6 border-b border-white/5 flex items-center justify-between"><h3 class="font-bold text-white">VPS 详细配置</h3><button id="cfg-close-x" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10"><div class="w-5 h-5 opacity-60">${ICONS.x}</div></button></div>' +
    ...
    '<div class="flex items-center justify-between mb-2"><span class="text-xs text-emerald-400 font-medium">\u90e8\u7f72\u8282\u70b9 (V2bX)</span></div>' +
    '<div class="text-xs text-slate-400 mb-1">\u2460 \u70b9\u51fb\u590d\u5236 SSH \u547d\u4ee4 \u2192 \u7c98\u8d34\u5230\u7ec8\u7aef\u8fde\u63a5 \u2192 \u8f93\u5165\u5bc6\u7801\uff08\u70b9\u201c\u590d\u5236\u5bc6\u7801\u201d\u53ef\u81ea\u52a8\u590d\u5236\uff09</div>' +
    '<div id="deploy-ssh-cmd" class="p-2 rounded bg-black/30 text-xs font-mono text-emerald-300 break-all cursor-pointer" title="\u70b9\u51fb\u590d\u5236">ssh ' + (v.username||'root') + '@' + v.ip + ' -p ' + v.port + '</div>' +
    '<div class="text-xs text-slate-400 mt-2 mb-1">\u2461 \u767b\u5f55\u540e\u6267\u884c\u5b89\u88c5\u547d\u4ee4</div>' +
    '<div id="deploy-install-cmd" class="p-2 rounded bg-black/30 text-xs font-mono text-emerald-300 break-all cursor-pointer" title="\u70b9\u51fb\u590d\u5236">bash &lt;(curl -sSL "https://gist.githubusercontent.com/hubentuan/b0719300ba5a41f9e828bbde0dc64733/raw/V2bX%20\u5b89\u88c5\u811a\u672c")</div>' +
    '</div>' +
    '<div class="p-3 rounded-lg bg-red-500/5 border border-red-500/10">' +
    '<div class="flex items-center justify-between mb-2"><span class="text-xs text-red-400 font-medium">\u5378\u8f7d\u8282\u70b9 (V2bX)</span></div>' +
    '<div class="text-xs text-slate-400 mb-1">\u767b\u5f55 VPS \u540e\u6267\u884c\u4ee5\u4e0b\u547d\u4ee4</div>' +
    '<div id="uninstall-cmd" class="p-2 rounded bg-black/30 text-xs font-mono text-red-300 break-all cursor-pointer" title="\u70b9\u51fb\u590d\u5236">systemctl stop V2bX; systemctl disable V2bX; rm -f /etc/systemd/system/V2bX.service; systemctl daemon-reload; rm -rf /etc/V2bX; rm -f /usr/bin/v2bx /usr/bin/V2bX; echo "V2bX \u5df2\u5378\u8f7d"</div>' +
    '</div></div>' +
    '</div></div>';
  document.body.appendChild(overlay);
  ...
  const sshCmd = 'ssh ' + (v.username || 'root') + '@' + v.ip + ' -p ' + v.port;
  document.getElementById('cfg-copy-ssh').addEventListener('click', () => {
    navigator.clipboard.writeText(sshCmd).then(() => toast('SSH \u547d\u4ee4\u5df2\u590d\u5236\uff0c\u7c98\u8d34\u5230\u7ec8\u7aef\u5373\u53ef\u8fde\u63a5','success')).catch(() => toast('\u590d\u5236\u5931\u8d25','error'));
  });
  document.getElementById('cfg-copy-pwd').addEventListener('click', () => {
    if (!secretValue) { toast('\u672a\u8bbe\u7f6e\u5bc6\u7801/\u5bc6\u94a5','warn'); return; }
    navigator.clipboard.writeText(secretValue).then(() => toast(isKey ? '\u79c1\u94a5\u5df2\u590d\u5236' : '\u5bc6\u7801\u5df2\u590d\u5236\uff0cSSH \u8fde\u63a5\u540e Ctrl+V \u7c98\u8d34\u5373\u53ef','success')).catch(() => toast('\u590d\u5236\u5931\u8d25','error'));
  });
  ...
  const UNINSTALL_CMD = 'systemctl stop V2bX; systemctl disable V2bX; rm -f /etc/systemd/system/V2bX.service; systemctl daemon-reload; rm -rf /etc/V2bX; rm -f /usr/bin/v2bx /usr/bin/V2bX; echo "V2bX \u5df2\u5378\u8f7d"';
  document.getElementById('deploy-ssh-cmd').addEventListener('click', () => {
    navigator.clipboard.writeText(sshCmd).then(() => toast('SSH \u547d\u4ee4\u5df2\u590d\u5236','success')).catch(() => toast('\u590d\u5236\u5931\u8d25','error'));
  });
